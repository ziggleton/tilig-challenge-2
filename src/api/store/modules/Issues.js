...
const issues = {
  state: {
    ...
    issue: null,
    setEmojis: () => {
      const arr = localStorage.getItem("setEmojis");
      if (arr) {
        return JSON.parse(localStorage.getItem("setEmojis"));
      } else {
        return null;
      }
    },
  ...
  mutations: {
    UPDATE_ISSUE(state, data) {
      let arr;
      if (!state.setEmojis()) {
        arr = new Array({ id: data.id, emoji: data.emoji });
        localStorage.setItem("setEmojis", JSON.stringify(arr));
      } else {
        arr = state.setEmojis();
        const item = arr.find((i) => parseInt(i.id) == data.id);
        if (!item) arr.push({ id: data.id, emoji: data.emoji });
        if (item) {
          const idx = arr.indexOf(item);
          arr[idx] = { ...item, emoji: data.emoji };
        }
        localStorage.setItem("setEmojis", JSON.stringify(arr));
      }
    },
  ...
  actions: {
    ...
    async getIssues({ commit }) {
      const res = await Gitservice.getIssues();
